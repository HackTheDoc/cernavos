...
UIButton::UIButton(const std::string& text, const Event::ID eid, const std::string& font, const SDL_Color& color, const Type type) {
    setID(eid);
    this->type = type;
...
void UIButton::setText(const std::string& text, const std::string& font, const SDL_Color& color) {
    ...
    hoveredTexture = Manager::GenerateText(t, font+" bold", color);
